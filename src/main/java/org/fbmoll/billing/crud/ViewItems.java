...
import org.fbmoll.billing.dataClasses.Item;
...
public class ViewItems {
    ...
    public static void showItemsTable(JPanel panel) throws SQLException {
        List<Item> articles = Queries.queryGetArticles();
        ...
        for (int i = 0; i < articles.size(); i++) {
            ...
            for (int j = 0; j < declaredFields.length; j++) {
                try {
                    data[i][j] = declaredFields[j].get(articles.get(i));
    ...
    private static JPanel createFilterPanel(JPanel panel, String[] columns, DefaultTableModel model, JTable table) {
        ...
        JButton createItemButton = new JButton("Crear Articulo");
        createItemButton.addActionListener(e -> new CreateItem().createItem(panel));
        ...
        return styleFilterPanel(filterLabel, filterField, columnSelector, createItemButton);
    }
